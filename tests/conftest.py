...
def test_functions(function_tests, globals_dict, instance=None):
    ...
    """
    # Define once so we can safely catch it later
    ...
    # For non-method tests, build once. For method tests, we'll deep-copy and rebuild per test case.
    precomputed_nonmethod_all_functions = None
    precomputed_nonmethod_context = None
    precomputed_nonmethod_names = None

    if instance is None:
        precomputed_nonmethod_all_functions = get_all_custom_functions(globals_dict)
        precomputed_nonmethod_context = "your code"
        precomputed_nonmethod_names = '\n'.join(precomputed_nonmethod_all_functions.keys())

    for test_case in function_tests:
        # Build function map & context
        if instance is not None:
            import copy
            is_method_test = True
            # fresh object per test case
            current_instance = copy.deepcopy(instance)
            # Testing methods of the fresh instance for this test_case
            all_functions = {
                name: getattr(current_instance, name) for name in dir(current_instance)
                if callable(getattr(current_instance, name)) and not name.startswith('__')
            }
            context = f"object of class {current_instance.__class__.__name__}"
            all_functions_names = '\n'.join(all_functions.keys())
        else:
            current_instance = None  # not used, keeps names explicit
            all_functions = precomputed_nonmethod_all_functions
            context = precomputed_nonmethod_context
            all_functions_names = precomputed_nonmethod_names
        ...
        if callable(func):
            ...
            try:
                for _ in range(num_calls):  # usually just called once, but some tests require several calls
                    if is_method_test:
                        # set variables on the *fresh* instance for this test_case
                        ...
                        if values_to_set:
                            ...
                            used_map, missing = apply_set_var_values(current_instance, values_to_set, create_if_missing=False)  # STRICT
                            ...
                            if missing:
                                ...
                                available = list(getattr(current_instance, "__dict__", {}).keys())
                        ...
                        object_state = get_object_state(current_instance)
                        test_case.setdefault('initial_obj_state', []).append(object_state)

                    actual_return_value = func(*args)  # bound to current_instance for method tests
